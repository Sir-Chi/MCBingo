...
$execute if score condensedTeams gameSetup matches 1 run tellraw @a [{"color":"$(team)","selector":"@s"}," obtained ",{"color":"gold","fallback":"$(special_item)","translate":"$(name)"},"! ",{"score":{"name":"@e[type=marker,tag=$(team),limit=1]","objective":"Score"}}," in ",{"score":{"name":"minutes","objective":"time"}}," : ",{"score":{"name":"seconds","objective":"time"}}]
...
$execute as @s[team=blue] run return run execute at @e[type=marker,tag=card_item,tag=$(special_item)] run place template bingo:card_extras/blue_lockout_card_marker ~-3 ~1 ~-4
$execute as @s[team=red] run return run execute at @e[type=marker,tag=card_item,tag=$(special_item)] run place template bingo:card_extras/red_lockout_card_marker ~-3 ~1 ~-4
$execute as @s[team=green] run return run execute at @e[type=marker,tag=card_item,tag=$(special_item)] run place template bingo:card_extras/green_lockout_card_marker ~-3 ~1 ~-4
$execute as @s[team=yellow] run return run execute at @e[type=marker,tag=card_item,tag=$(special_item)] run place template bingo:card_extras/yellow_lockout_card_marker ~-3 ~1 ~-4