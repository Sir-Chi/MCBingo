...
execute at @e[type=item,tag=droppedMap] unless entity @a[tag=playerThatIsMapUpdating] unless entity @a[tag=oneGuyToTeleport] unless entity @a[tag=blueGuyToTeleport] unless entity @a[tag=redGuyToTeleport] unless entity @a[tag=greenGuyToTeleport] unless entity @a[tag=yellowGuyToTeleport] run tag @p[distance=0..5,tag=!playerThatWantsToUpdate,tag=!playerThatIsMapUpdating,limit=1] add playerThatWantsToUpdate
