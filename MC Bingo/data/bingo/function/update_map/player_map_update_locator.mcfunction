...
$execute at @s unless entity @s[nbt={Dimension:"bingo:bingo_card"},team=$(team),tag=$(team)PlayerThatIsMapUpdating,x=-3,y=241,z=-7,dx=6,dy=6,dz=6] run summon marker ~ ~ ~ {Tags:["$(team)WhereToTpBackTo"]}
...
$execute as @s run function bingo:update_map/teleport_to_card {team:'$(team)'}