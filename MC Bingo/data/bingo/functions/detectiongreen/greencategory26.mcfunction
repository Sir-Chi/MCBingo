...
execute if score Teams GameSetup matches 0 if score Lockout GameSetup matches 0 if score rail GreenItems matches 1 run tellraw @p ["",{"selector":"@a[team=green]","color":"green"},{"text":" obtained "},{"text":"Rail! ","color":"gold"},{"score":{"name":"@p[team=green]","objective":"Score"}}," in ",{"score":{"name":"minutes","objective":"timer"}}," : ",{"score":{"name":"seconds","objective":"timer"}}]
...
execute if score Lockout GameSetup matches 1 if score rail GreenItems matches 1 if score rail RedItems matches 0 if score rail BlueItems matches 0 if score rail YellowItems matches 0 run execute at @a run playsound entity.firework_rocket.launch ambient @p ~ ~ ~ 1 1 1
