...
execute if score Teams GameSetup matches 0 if score Lockout GameSetup matches 0 if score bottle RedItems matches 1 run tellraw @p ["",{"selector":"@a[team=red]","color":"red"},{"text":" obtained "},{"text":"Glass Bottle! ","color":"gold"},{"score":{"name":"@p[team=red]","objective":"Score"}}," in ",{"score":{"name":"minutes","objective":"time"}}," : ",{"score":{"name":"seconds","objective":"time"}}]
...
execute if score Lockout GameSetup matches 1 if score bottle RedItems matches 1 if score bottle BlueItems matches 0 if score bottle GreenItems matches 0 if score bottle YellowItems matches 0 run execute at @a run playsound entity.firework_rocket.launch ambient @p ~ ~ ~ 1 1 1
