...
execute if score YellowTeam TeamCount matches 1 at @p[tag=oneGuyToTeleport] run tp @a[tag=oneGuyToTeleport] ~ 260 ~
execute if score PVP GameSetup matches 1 if score PVP GameSetup matches 1 at @p[tag=oneGuyToTeleport] run tp @a[tag=oneGuyToTeleport] ~-200 260 ~
execute if score PVP GameSetup matches 0 if score YellowTeam TeamCount matches 1 run data merge block -29 278 -9 {auto:1b}
