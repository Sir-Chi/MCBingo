...
execute as @e[tag=Z,scores={Z=0}] at @e[type=area_effect_cloud,tag=findblock] run tp @e[type=area_effect_cloud,tag=findblock] ~ ~1 ~
scoreboard players operation @e[tag=Z] Z += x1 S
execute as @e[tag=findblock] at @e[tag=findblock] positioned ~ ~ ~ run clone ~ ~ ~ ~ ~ ~ -29 280 -9
...
execute as @e[tag=findblock] at @e[tag=findblock] positioned ~ ~ ~ run clone ~ ~ ~ ~ ~ ~ -7 273 -16

kill @e[tag=findblock]

schedule function bingo:start/xzyellow 2t